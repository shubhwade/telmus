...
from valkit.core.result import GrowthResult
...
class GrowthEngine:
    ...
    def _margin_trend(self, income_stmt: pd.DataFrame) -> str | None:
        ...
        if len(revenue) < 3 or len(operating_income) < 3:
            logger.warning("Margin trend requires 3 years of revenue and operating income")
            return None
        margins = []
        for i in range(3):
    ...
    def _fcf_yield(self, cashflow: pd.DataFrame, info: dict[str, object]) -> float | None:
        cash_from_ops_series = _get_series(cashflow, "Total Cash From Operating Activities")
        ...
        if cash_from_ops is None or capex is None or market_cap is None or market_cap == 0:
