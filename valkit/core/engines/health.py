...
from valkit.core.result import HealthResult
...
class HealthEngine:
    ...
    def _roa_positive(self, income_stmt: pd.DataFrame, balance_sheet: pd.DataFrame) -> bool:
    ...
    def _roa_increasing(self, income_stmt: pd.DataFrame, balance_sheet: pd.DataFrame) -> bool:
        ...
        if len(net_income) < 2 or len(total_assets) < 2:
            ...
            return False
        prior_roa = _safe_divide(_safe_value(net_income.iloc[1]), _safe_value(total_assets.iloc[1]))
        current_roa = _safe_divide(_safe_value(net_income.iloc[0]), _safe_value(total_assets.iloc[0]))
    ...
    def _accruals(self, income_stmt: pd.DataFrame, cashflow: pd.DataFrame) -> bool:
        net_income = _series_value(_get_series(income_stmt, "Net Income"), 0)
        cfo = _series_value(_get_series(cashflow, "Total Cash From Operating Activities"), 0)
    ...
    def _leverage_decreasing(self, balance_sheet: pd.DataFrame) -> bool:
        ...
        if len(long_term_debt) < 2 or len(total_equity) < 2:
            ...
            return False
        current_de = _safe_divide(_series_value(long_term_debt, 0), _series_value(total_equity, 0))
        prior_de = _safe_divide(_series_value(long_term_debt, 1), _series_value(total_equity, 1))
    ...
    def _liquidity_increasing(self, balance_sheet: pd.DataFrame) -> bool:
        ...
        if len(current_assets) < 2 or len(current_liabilities) < 2:
            ...
            return False
        current_ratio = _safe_divide(_series_value(current_assets, 0), _series_value(current_liabilities, 0))
        prior_ratio = _safe_divide(_series_value(current_assets, 1), _series_value(current_liabilities, 1))
    ...
    def _gross_margin_increasing(self, income_stmt: pd.DataFrame) -> bool:
        ...
        if len(revenue) < 2 or len(gross_profit) < 2:
            ...
            return False
        prior_margin = _safe_divide(_series_value(gross_profit, 1), _series_value(revenue, 1))
        current_margin = _safe_divide(_series_value(gross_profit, 0), _series_value(revenue, 0))
    ...
    def _asset_turnover_increasing(self, income_stmt: pd.DataFrame, balance_sheet: pd.DataFrame) -> bool:
        revenue = _get_series(income_stmt, "Total Revenue")
        total_assets = _get_series(balance_sheet, "Total Assets")
        ...
        if len(revenue) < 2 or len(total_assets) < 2:
            ...
            return False
        prior_turnover = _safe_divide(_series_value(revenue, 1), _series_value(total_assets, 1))
        current_turnover = _safe_divide(_series_value(revenue, 0), _series_value(total_assets, 0))
    ...
    def _compute_altman_z(self, income_stmt: pd.DataFrame, balance_sheet: pd.DataFrame, info: dict[str, object]) -> float | None:
        total_assets = _safe_value(_series_value(_get_series(balance_sheet, "Total Assets"), 0))
        ...
        current_assets = _series_value(_get_series(balance_sheet, "Total Current Assets"), 0)
        current_liabilities = _series_value(_get_series(balance_sheet, "Total Current Liabilities"), 0)
        if current_assets is not None and current_liabilities is not None:
            working_capital = _safe_value(current_assets - current_liabilities)

        retained_earnings = _safe_value(_series_value(_get_series(balance_sheet, "Retained Earnings"), 0))
        if retained_earnings is None:
            retained_earnings = _safe_value(_series_value(_get_series(balance_sheet, "Total Stockholder Equity"), 0))
        if retained_earnings is None:
            retained_earnings = 0.0

        ebit = _safe_value(_series_value(_get_series(income_stmt, "Ebit"), 0))
        if ebit is None:
            ebit = _safe_value(_series_value(_get_series(income_stmt, "Operating Income"), 0))
        if ebit is None:
            ebit = _safe_value(_series_value(_get_series(income_stmt, "Ebitda"), 0))

        total_liabilities = _safe_value(_series_value(_get_series(balance_sheet, "Total Liab"), 0))
        if total_liabilities is None:
            total_liabilities = _safe_value(_series_value(_get_series(balance_sheet, "Total Liabilities Net Minority Interest"), 0))

        revenue = _safe_value(_series_value(_get_series(income_stmt, "totalRevenue"), 0))
        if revenue is None:
            revenue = _safe_value(_series_value(_get_series(income_stmt, "Total Revenue"), 0))
    ...
    def _compute_debt_to_equity(self, balance_sheet: pd.DataFrame) -> float | None:
        total_liab = _safe_value(_series_value(_get_series(balance_sheet, "Total Liab"), 0))
        total_equity = _safe_value(_series_value(_get_series(balance_sheet, "Total Stockholder Equity"), 0))
    ...
    def _compute_current_ratio(self, balance_sheet: pd.DataFrame) -> float | None:
        current_assets = _safe_value(_series_value(_get_series(balance_sheet, "Total Current Assets"), 0))
        current_liabilities = _safe_value(_series_value(_get_series(balance_sheet, "Total Current Liabilities"), 0))
        if current_assets is None or current_liabilities is None:
    ...
    def _compute_interest_coverage(self, income_stmt: pd.DataFrame, balance_sheet: pd.DataFrame) -> float | None:
        ebit = _safe_value(_series_value(_get_series(income_stmt, "Ebit"), 0))
        if ebit is None:
            ebit = _safe_value(_series_value(_get_series(income_stmt, "Operating Income"), 0))
        if ebit is None:
            ebit = _safe_value(_series_value(_get_series(income_stmt, "Ebitda"), 0))
        interest_expense = _safe_value(_series_value(_get_series(income_stmt, "Interest Expense"), 0))
