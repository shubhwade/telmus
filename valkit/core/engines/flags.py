...
from valkit.core.result import FlagsResult, RedFlag
...
class FlagsEngine:
    def run(self, financials: dict[str, object]) -> FlagsResult:
        ...
        if beneish_m is not None and beneish_m > -2.22:
            red_flags.append(RedFlag(type="beneish_m", value=round(beneish_m, 2), severity="high"))

        de_ratio = self._de_ratio(balance_sheet)
        if de_ratio is not None and de_ratio > 2.0:
            red_flags.append(RedFlag(type="de_ratio", value=round(de_ratio, 2), severity="medium"))

        negative_fcf_count = self._negative_fcf_years(cashflow)
        if negative_fcf_count >= 2:
            red_flags.append(RedFlag(type="negative_fcf", value=negative_fcf_count, severity="medium"))
        ...
        return FlagsResult(red_flags=red_flags, beneish_m=beneish_m, highest_concern=highest)
    ...
    def _beneish_m_score(
        ...
        if "Gross Profit" not in income_stmt.index or "Total Revenue" not in income_stmt.index:
            logger.warning("Beneish score missing revenue or gross profit")
            return None

        if _get_series(balance_sheet, "Total Assets") is None or _get_series(balance_sheet, "Total Current Assets") is None:
        ...
        try:
            ...
            net_income0 = val(net_income_series, 0) if net_income_series is not None else None
            ...
            depi = _safe_divide(
                _safe_divide(1 - _safe_divide(dep0, dep0 + gp0), 1 - _safe_divide(dep1, dep1 + gp1)),
                1,
            )
            sgai = _safe_divide(_safe_divide(sga0, rev0), _safe_divide(sga1, rev1))
            tata = _safe_divide(
                _safe_value(cfo0 - net_income0) if cfo0 is not None and net_income0 is not None else None,
                cfo0,
            )
            lvgi = _safe_divide(
