...
from valkit.core.result import ValuationResult
...
class ValuationEngine:
    def run(self, financials: dict[str, object]) -> ValuationResult:
        ...
        ev_ebitda = None
        if market_cap is not None and total_debt is not None and cash is not None and ebitda is not None:
    ...
    def _total_debt(self, balance_sheet: pd.DataFrame) -> float | None:
        ...
        if long_term is not None or current_debt is not None:
            return float((_safe_value(long_term.iloc[0]) if long_term is not None else 0.0) + (_safe_value(current_debt.iloc[0]) if current_debt is not None else 0.0))
        logger.warning("Total debt not found for EV/EBITDA calculation")
        return None

    def _cash(self, balance_sheet: pd.DataFrame) -> float | None:
        cash_series = _get_series(balance_sheet, "Cash") or _get_series(balance_sheet, "Cash And Cash Equivalents")
    ...
    def _sector_pe_median(self, sector: str, ticker: object | None = None) -> float | None:
        try:
