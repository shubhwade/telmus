...
from valkit.mcp.server import run_mcp_server

app = typer.Typer(help="valkit financial statement analysis CLI")
console = Console()
...
def _severity_color(flag: str | None) -> str:
    ...
    if flag == "expensive relative to sector" or "negative" in text or "weak" in text or "distress" in text:
        return "red"
    return "yellow"


def _render_section(name: str, metrics: list[tuple[str, object | None, str | None]]) -> Table:
...
def _print_scan(result: ScanResult) -> None:
    console.rule(f"[bold blue]{result.company}[/] — [green]{result.ticker}[/] — [cyan]{result.exchange}[/]")
...
def scan(
    ...
    export: str | None = typer.Option(None, "--export", help="Save result JSON to file."),
) -> None:
...
def compare(ticker_a: str, ticker_b: str) -> None:
    ...
    try:
        ...
        row("P/E ratio", comparison.result_a.valuation.pe_ratio, comparison.result_b.valuation.pe_ratio)
        row("P/B ratio", comparison.result_a.valuation.pb_ratio, comparison.result_b.valuation.pb_ratio)
        row("EV/EBITDA", comparison.result_a.valuation.ev_ebitda, comparison.result_b.valuation.ev_ebitda)
        row("Piotroski F-score", comparison.result_a.health.piotroski_f, comparison.result_b.health.piotroski_f)
        row("Altman Z-score", comparison.result_a.health.altman_z, comparison.result_b.health.altman_z)
        row("Revenue CAGR 3y", comparison.result_a.growth.revenue_cagr_3y, comparison.result_b.growth.revenue_cagr_3y)
        row("FCF yield", comparison.result_a.growth.fcf_yield, comparison.result_b.growth.fcf_yield)
...
def screen(
    ...
    for ticker in tickers:
        try:
            result = scanner.scan(ticker)
            if result.health.piotroski_f is None or result.health.debt_to_equity is None:
                continue
            if result.health.piotroski_f >= min_piotroski and result.health.debt_to_equity <= max_de:
                rows.append((ticker, result.health.piotroski_f, result.health.debt_to_equity, result.highest_concern))
        except Exception:
            continue
    table = Table(show_header=True, header_style="bold magenta")
...
def info() -> None:
    """Print package info."""
    try:
        package_version = version("valkit")
    except Exception:
...
def check(ticker: str) -> None:
    ...
    try:
        scanner = ValkitScanner()
        result = scanner.scan(ticker)
        console.print(f"Piotroski F-score: [bold]{result.health.piotroski_f or 'n/a'}[/bold]")
