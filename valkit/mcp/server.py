...
def _scan_and_serialize(scanner: ValkitScanner, ticker: str) -> str:
...
async def handle_tool_call(name: str, arguments: dict[str, Any] | None) -> types.CallToolResult:
    arguments = arguments or {}
    if name == "scan":
        ticker = arguments.get("ticker")
        if not ticker:
            raise ValueError("Missing ticker")
        payload = {"result": json.loads(_scan_and_serialize(ValkitScanner(), ticker))}
        ...
        comparison = ValkitScanner().compare(ticker_a, ticker_b)
